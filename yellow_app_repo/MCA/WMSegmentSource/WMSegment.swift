...
open class WMSegment: UIControl {
    ...
    @IBInspectable
    public var WMSegmentborderWidth: CGFloat = 0 {
        didSet {
    ...
    @IBInspectable
    public var WMSegmentborderColor: UIColor = .clear {
        didSet {
    ...
    @IBInspectable
    public var WMSegmentcornerRadius: CGFloat = 0 {
        didSet {
    ...
    func setupSelector() {
        ...
        if selectorType == .normal {
            ...
            selector = UIView(frame: CGRect(x: 20, y: frame.height - 3, width: selectorWidth - 40, height: 3))
            selector.layer.cornerRadius = 0
        }
            ...
            if btn == sender {
                ...
                if self.animate {
                    UIView.animate(withDuration: 0.25, delay: 0, options: .curveEaseOut, animations: {
                        self.selector.frame.origin.x = startPosition + 20
                    }, completion: nil)
                }else{
                    self.selector.frame.origin.x = startPosition + 20
                }
